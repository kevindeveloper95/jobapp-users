...
const securityMiddleware = (app: Application): void => {
  ...
  app.use(
    cors({
      origin: config.API_GATEWAY_URL, // ← Solo acepta peticiones del Gateway
      credentials: true,
  ...
  app.use((req: Request, _res: Response, next: NextFunction) => {
    if (req.headers.authorization) {
      ...
      req.currentUser = payload; // ← AQUÍ se procesa el token del Gateway
    }
    next();
  });
};
...
const routesMiddleware = (app: Application): void => {
  appRoutes(app); // ← AQUÍ se registran las rutas
};
